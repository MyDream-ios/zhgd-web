<template>
  <div id="systemSet_equipmentSet_towerCrane">
    <div class="content-box">
      <!-- 头部 -->
      <div class="title">
        <div class="text">塔吊设置</div>
      </div>
      <!-- 导航栏 -->
      <div class="nav">
      ...
      <div class="main" v-show="navState">
        <ul>
          <li v-for="item in craneList" :key="item.id">
            <div class="left-box">
              <div class="name">{{ item.craneName }}</div>
              <div class="location">
                <span>设备SN：</span>
                {{ item.hxzId }}
              </div>
        ...
        <div class="form">
          <ul>
            ...
            <li>
              ...
              <input type="number" v-model="tcLoadCapacity" placeholder="单位为Kg">
            </li>
            <li>
              <span>
                起重力矩
        ...
        <div class="form">
          <ul>
            ...
            <li>
              ...
              <input type="text" v-model="editTower.craneName">
            </li>
            <li>
              <span>
                设备SN
            ...
            <li>
              ...
              <input type="number" v-model="editTower.tcLoadCapacity" placeholder="单位为Kg">
            </li>
            <li>
              <span>
                起重力矩
...
<style lang="less">
#systemSet_equipmentSet_towerCrane {
  width: 100%;
  .content-box {
    min-height: 7rem;
...
export default {
  ...
  methods: {
    ...
    deleteTower(item) {
      if (item.scznl == 'CAY') {
        ...
        this.$confirm('此操作将永久删除该塔吊, 是否继续?', '提示', {
          ...
          this.$axios
            .post(`/api/OptionsCraneApi/deleteCrane?id=${item.id}`)
            .then(res => {
              if (res.data.code == 0) {
                this.$message({
                  message: '删除成功',
    ...
    deleteCAY(id) {
      if (this.deleteTime) {
        this.$axios
          .post(`/api/OptionsCraneApi/deleteCrane?id=${this.deleteId}&devCcrq=${this.deleteTime}`)
          .then(res => {
            if (res.data.code == 0) {
              this.$message({
                message: '删除成功',
    ...
    getCraneUserList() {
      this.$axios
        .post(`/api/OptionsCraneApi/getCraneUserList?projectId=${this.projectId}`)
        .then(res => {
    ...
    switchClick(obj) {
      ...
      this.$axios
        .post(`/api/OptionsPushApi/optionsPush?privilegesId=7&userId=${obj.id}&onOff=${obj.onOff?1:0}`)
        .then(res => {
          if (res.data.code === 0) {
            this.messageBox(res.data.msg, 1)
          } else {
            this.messageBox('推送开启失败', 0)
          }
        })
    },
    ...
    searchClick() {
      this.$axios
        .post(`/api/OptionsCraneApi/getCraneUserList?projectId=${this.projectId}&filed=${this.search}`)
        .then(res => {
          if (res.data.code == 0) {
            for (let i = 0; i < res.data.data.length; i++) {
              if (res.data.data[i].onOff == 0) {
