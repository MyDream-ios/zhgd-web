      ...
      <div class="main-box">
        ...
        <div class="table-box">
          <el-table :data="tableData" stripe border @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="licence" label="社会统一信用代码" width="150"></el-table-column>
            <el-table-column prop="name" label="参建单位名称" width="200"></el-table-column>
            <el-table-column prop="abbreviation" label="简称" width="80"></el-table-column>
            <el-table-column prop="type" label="单位类型" width="100"></el-table-column>
          ...
          <ul>
            ...
            <li>
              <div>
                ...
                <el-select v-model="isUpload" placeholder="请选择">
                  <el-option
                    v-for="item in isUploadList"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  ></el-option>
  ...
  methods: {
      ...
      if (this.selectionId == "") {
        ...
        this.$confirm("是否要删除选中的参建单位？", "提示", {
          ...
          .then(() => {
            this.$axios
              .post(
                `/api/constructionCompanyApi/deleteConstructionCompanyIds?ids=${this.selectionId}`
              )
              .then(res => {
    ...
    insertConstructionCompany() {
      if (
        ...
        this.$axios
          .post(
            `/api/constructionCompanyApi/insertConstructionCompany?projectId=${this.pid}&constructionName=${this.constructionName}&shortName=${this.shortName}&capital=${this.capital}&companyType=${this.companyType}&legalPerson=${this.legalPerson}&suid=${this.suid}&bankOpen=${this.bankOpen}&bankNum=${this.bankNum}&bankAddress=${this.bankAddress}&address=${this.address}&contacts=${this.contacts}&mobilePhone=${this.mobilePhone}&email=${this.email}&remark=${this.remark}&isUpload=${this.isUpload}`
          )
          .then(res => {
            if (res.data.code == 0) {
              this.$message({
    ...
    selectConstructionCompanyList() {
      this.$axios
        .post(
          `/api/constructionCompanyApi/selectConstructionCompanyList?projectId=${this.pid}&pageNum=${this.pageNum}&pageSize=${this.pageSize}`
        )
        .then(res => {
          // console.log(res.data)
          let temp = [];
    ...
    selectConstructionCompanyId() {
      this.$axios
        .post(
          `/api/constructionCompanyApi/selectConstructionCompanyId?id=${Number(
            this.selectionId
          )}`
        )
        .then(res => {
          ...
          this.isUpload = res.data.data.isUpload ? res.data.data.isUpload : "";
        });
    },
    ...
    updateConstructionCompany() {
      if (
        ...
        this.$axios
          .post(
            `/api/constructionCompanyApi/updateConstructionCompany?id=${Number(
              this.selectionId
            )}&constructionName=${this.constructionName}&shortName=${
              this.shortName
            }&capital=${this.capital}&companyType=${
              this.companyType
            }&legalPerson=${this.legalPerson}&suid=${this.suid}&bankOpen=${
              this.bankOpen
            }&bankNum=${this.bankNum}&bankAddress=${this.bankAddress}&address=${
              this.address
            }&contacts=${this.contacts}&mobilePhone=${this.mobilePhone}&email=${
              this.email
            }&remark=${this.remark}&isUpload=${this.isUpload}`
          )
          .then(res => {
            console.log(res.data);
            if (res.data.code == 0) {
    ...
    searchClick() {
      ...
      this.$axios
        .post(
          `/api/constructionCompanyApi/selectConstructionCompanyList?projectId=${this.pid}&pageNum=${this.pageNum}&pageSize=${this.pageSize}&param=${this.param}&suid=${this.searchSuid}`
        )
        .then(res => {
          // console.log(res.data)
          let temp = [];
