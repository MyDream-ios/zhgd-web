    ...
    <div class="centent-box">
          ...
          <el-table :data="tableData" stripe border>
            ...
            <el-table-column prop="state" label="状态" width="200">
              <template slot-scope="scope">
                <div
                  :class="scope.row.state==0?'green-color':scope.row.state==2?'yellow-color':'red-color'"
                >{{ scope.row.state==0?'在建':scope.row.state==2?'竣工':'停工' }}</div>
              </template>
            </el-table-column>
      ...
      <div class="dialog-box" v-show="dialogShow">
          ...
          <ul>
            ...
            <li>
              <div class="left-box">
                ...
                <input type="number" v-model="projectNumber" />
              </div>
            ...
            <li>
              <div class="left-box">
                <div class="text-box">安全报检编号</div>
                <input type="text" v-model="securityCode" />
              </div>
              <div class="right-box">
                <div class="text-box">质量报检编号</div>
            ...
            <li>
              <div class="left-box">
                <div class="text-box">设计单位</div>
                <input type="text" v-model="designOrganization" />
              </div>
              <div class="right-box">
                <div class="text-box">勘察单位</div>
                <input type="text" v-model="explorationUnit" />
              </div>
            </li>
            <li>
              <div class="text-box">备注</div>
              <input type="text" v-model="remark" />
            </li>
            <li class="upload-pic">
              <div class="text-box">项目效果图</div>
              <a>点击上传</a>
            </li>
          </ul>
        </div>
        ...
        <div class="confirm" v-show="dialogPageShow">
          ...
          <a class="button" @click="addProject">确定</a>
        </div>
        <div class="confirm" v-show="!dialogPageShow">
...
export default {
  ...
  methods: {
    ...
    getArea() {
      this.$axios.post(`/api/area/getArea?parentId=0`).then(res => {
        ...
        for (let i = 0; i < res.data.data.length; i++) {
          this.$axios
            .post(`/api/area/getArea?parentId=${res.data.data[i].id}`)
            .then(res2 => {
              // console.log(res2.data.data)
              let temp2 = [];
              for (let i2 = 0; i2 < res2.data.data.length; i2++) {
                this.$axios
                  .post(`/api/area/getArea?parentId=${res2.data.data[i2].id}`)
                  .then(res3 => {
    ...
    selectProjectList() {
      this.$axios
        .post(
          `/api/project/selectProjectList?cid=${this.cid}&pageNum=${this.pageNum}&pageSize=${this.pageSize}`
        )
        .then(res => {
          // console.log(res.data)
          let temp = [];
    ...
    selectProjectType() {
      this.$axios
        .post(`/api/dictionariesApi/selectDictionaries?category=PROJECT_TYPE`)
        .then(res => {
          // console.log(res.data)
    ...
    selectProjectState() {
      this.$axios
        .post(`/api/dictionariesApi/selectDictionaries?category=PROJECT_STATE`)
        .then(res => {
          // console.log(res.data);
    ...
    deleteSystemUser(id) {
      this.$confirm("是否要删除选中的账号？", "提示", {
        ...
        .then(() => {
          this.$axios.post(`/api/project/remove?ids=${id}`).then(res => {
            // console.log(res.data)
            if (res.data.code == 0) {
              this.$message({
                type: "success",
    ...
    selectUserId() {
      this.$axios
        .post(`/api/project/getProject?projectId=${this.id}`)
        .then(res => {
          ...
          this.securityCode = res.data.data.securityCode
            ? res.data.data.securityCode
            : "";
          this.qualityNumber = res.data.data.qualityNumber
            ? res.data.data.qualityNumber
            : "";
          this.designOrganization = res.data.data.designOrganization
            ? res.data.data.designOrganization
            : "";
          this.explorationUnit = res.data.data.explorationUnit
            ? res.data.data.explorationUnit
            : "";
    ...
    searchClick() {
      this.$axios
        .post(
          `/api/project/selectProjectList?cid=${this.cid}&pageNum=1&pageSize=${this.pageSize}&projectName=${this.searchName}`
        )
        .then(res => {
          // console.log(res.data)
          let temp = [];
    ...
    pageClick() {
      this.$axios
        .post(
          `/api/project/selectProjectList?cid=${this.cid}&pageNum=${this.pageNum}&pageSize=${this.pageSize}&projectName=${this.searchName}`
        )
        .then(res => {
          // console.log(res.data)
          let temp = [];
    ...
    selectPlatform() {
      this.$axios
        .post(`/api/dictionariesApi/selectDictionaries?category=PLATFORM`)
        .then(res => {
          // console.log(res.data)
    ...
    selectKeyType() {
      this.$axios
        .post(`/api/dictionariesApi/selectDictionaries?category=KEY_TYPE`)
        .then(res => {
          // console.log(res.data)
    ...
    addSecretKey() {
      if (
        ...
        this.$axios
          .post(
            `/api/synchronizationInformationApi/add?projectNumber=${this.projectNumber}&apiKey=${this.apiKey}&apiSecret=${this.apiSecret}&clientSerial=${this.clientSerial}&projectId=${this.projectId}&engineeringCode=${this.engineeringCode}&platformName=${this.platformName}&state=${this.state}&apiType=${this.apiType}`
          )
          .then(res => {
            // console.log(res.data)
            if (res.data.code == 0) {
    ...
    keyList(projectId) {
      ...
      this.$axios
        .post(
          `/api/synchronizationInformationApi/list?projectId=${projectId}&pageNum=1&pageSize=99`
        )
        .then(res => {
          console.log(res.data);
        });
